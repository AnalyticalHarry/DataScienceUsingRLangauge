...
# 2. View tab for showing the dataset
View(dataset)
...
# 6. Function will return the internal structure of an R object
str(dataset)

# 7. Function will print unique column names and data types
...
# 16. Generalized Least Squares (GLS) Model for Multiple Regression
gls_model <- gls(train_y ~ ., data = data.frame(train_X, train_y))

# Summarize the GLS model
summary(gls_model)

# 17. Predicting using the model
train_pred <- predict(gls_model, newdata = train_X)
test_pred <- predict(gls_model, newdata = test_X)
